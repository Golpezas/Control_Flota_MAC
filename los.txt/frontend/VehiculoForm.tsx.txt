...
const VehiculoForm: React.FC<VehiculoFormProps> = ({ onSuccess, initialData, isEditMode = false }) => {
  ...
  // 🔑 CORRECCIÓN: Renombramos 'e' a '_e' para silenciar el warning 'no-unused-vars'.
  const handleSubmit = async (_e: FormEvent) => {
    _e.preventDefault(); // Usamos '_e' aquí para prevenir la recarga.
    ...
    try {
        if (isEditMode) {
            ...
            // Creamos el payload de actualización excluyendo la patente
