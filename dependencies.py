...
from pydantic import BaseModel, Field, ConfigDict, Field, field_validator
...
class CostoManualInput(BaseModel):
    ...
    @classmethod
    def normalize_patente_val(cls, v: str) -> str:
        """Normaliza la patente usando la función compartida en dependencies.py."""
        normalized = normalize_patente(v)
        if not normalized:
            raise ValueError("Patente inválida: debe contener al menos un carácter alfanumérico.")
        return normalized
    ...
    @classmethod
    def round_importe(cls, v: float) -> float:
        """Redondea el importe a 2 decimales para consistencia financiera."""
        return round(v, 2)
