...
# 💡 NUEVO ENDPOINT: RESUMEN DE INFRACCIONES
@router.get("/infracciones/resumen")
async def get_resumen_infracciones(
    ...
    cursor_finanzas = db_finanzas.find({})
    ...
    for doc in docs_finanzas:
        ...
        tipo_registro = str(doc.get("tipo_registro", "")).upper()
        
        es_infraccion = False
        if tipo_registro == "INFRACCION":
            es_infraccion = True
        elif any(palabra in motivo for palabra in ["MULTA", "INFRACCION", "EXCESO", "VELOCIDAD", "VIA PROHIBIDA", "SEMÁFORO", "SEMAFORO"]):
            es_infraccion = True
        ...
        if es_infraccion:
            ...
            if pat not in infracciones_dict:
                infracciones_dict[pat] = {"cantidad": 0, "total": 0.0}
            infracciones_dict[pat]["cantidad"] += 1
            infracciones_dict[pat]["total"] += monto

    resumen = []
    for pat, data in infracciones_dict.items():
        if patente and patente.upper() not in pat.upper():
            continue 
            
        veh = await db_vehiculos.find_one({"_id": pat})
        if veh:
            nro_movil = veh.get("nro_movil") or veh.get("NRO_MOVIL") or "S/N"
            marca = veh.get("marca") or veh.get("MARCA") or ""
            modelo = veh.get("modelo") or veh.get("MODELO") or veh.get("descripcion_modelo") or veh.get("DESCRIPCION_MODELO") or "Sin modelo"
            desc_modelo = f"{marca} {modelo}".strip()
        else:
            nro_movil = "S/N"
            desc_modelo = "Desconocido"

        resumen.append({
            "patente": pat,
            "nro_movil": str(nro_movil),
            "descripcion_modelo": desc_modelo,
            "cantidad_infracciones": data["cantidad"],
            "total_adeudado": data["total"]
        })

    # Ordenamos por total de la deuda
    resumen.sort(key=lambda x: x["total_adeudado"], reverse=True)
    
    total_items = len(resumen)
    resumen_paginado = resumen[skip : skip + limit]

    return {"infracciones": resumen_paginado, "total": total_items}
