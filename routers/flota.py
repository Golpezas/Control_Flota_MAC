    ...
    if remaining > 0:
        ...
        for veh in vehiculos:
            ...
            for doc in veh.get('documentos_digitales', []):
                ...
                # Descartamos inmediatamente lo que no urge
...
async def get_alertas_criticas(
    ...
    """
    Devuelve alertas paginadas + total real con filtro de fechas aplicado.
    """
    alertas = await get_vencimientos_criticos_alertas(dias_tolerancia, skip, limit, patente)

    # =================================================================
    # CÁLCULO DE TOTAL REAL (ESTRICTO)
    # =================================================================
    db_documentacion = get_db_collection("Documentacion")
    ...
    filtro_total = {
        "tipo_documento": {"$in": ["SEGURO", "Poliza_Detalle", "VTV"]},
        "fecha_vencimiento": {"$ne": None, "$lte": fecha_limite} # ← EL FILTRO ASEGURA EL TOTAL EXACTO
    }
    ...
    # Enriquecimiento final de metadatos (Si quedaron huérfanos)
    db_vehiculos = get_db_collection("Vehiculos")
