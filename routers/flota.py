...
async def delete_costo_manual(
    ...
    delete_result = await collection.delete_one({"_id": obj_id})  # ← CAMBIO: await
...
async def limpiar_costos_basura(
    ...
    for col_name in colecciones:
        ...
        if dry_run:
            count = await collection.count_documents(filtro)  # ← CAMBIO: await
            detalles[col_name] = count
        else:
            resultado = await collection.delete_many(filtro)  # ← CAMBIO: await
